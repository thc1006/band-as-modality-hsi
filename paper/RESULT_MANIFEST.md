...
| Formal weighted CRC (test-point term) | real L2A 1.87@7% DEPLOYMENT mean, **feasible only 12%** (P0-3 FIXED: `weighted_crc_perunit` returns a feasibility flag — an infeasible unit, target<0, is an abstain-FALLBACK, NOT a formal 0-risk; conditional-on-feasible 15.4@53 reported separately); est-w PC 7.1 tracks true 6.2 @ AUROC 0.88 overlap | `phase8R11_weighted_crc_formal.py` | `paper/results_phase8R11_estweight.log` |
...
| Radiometric audit (magnitude self-consistency) | L1C p1≈15 ⇒ under a +1000 offset the true refl there would be ≈−0.10, far beyond dark-target noise (~−0.001) ⇒ stored on no-offset scale; all versions pre-N04.00; and the product-aware fix is offset-invariant anyway, so the headline does not rest on this audit (physics wording corrected in review: TOA refl CAN be slightly negative — a magnitude test, not a sign impossibility) | `phase8R12_radiometric_audit.py` | (stdout) |
...
| P0-2 quantile-tie convention — FIXED IN CODE | `phase8R10.quantile_match` now uses the tie-aware mid-distribution CDF on unique knots (`_mid_cdf_knots`) + a self-test (identity 9.5e-08, monotone, tie F(0)=0.375) + per-band diagnostics (knots/clamp/endpoint); R17/R18/R19 inherit it. On the full 10×10 it lowers the quantile-transport joint ~0.1–0.2 pp (R10 9.6→9.5, R17 9.55→9.35) — within the pipeline's GPU run-to-run non-determinism (non-deterministic attention backward, see `bandsim/parallel.py`) and the ±0.3 SE; the product-aware (mean/std) headline **10.8 and every claim unchanged**. Committed R10/R17/R18 CSVs are the released first-rank draw the paper cites (9.6); the corrected code reproduces the within-noise-lower values on re-run | `phase8R10_normalization_control.py`; probe `scratchpad/probe_tie.py` | (code fix; paper numbers stand) |
...
| Quantile transfer: acquisition-month + Sen2Cor product-version (LOOSE split) | full 9.55; Mar–Aug/Sep–Feb 10.4/9.8; new/old Sen2Cor product-version 9.9/10.8; both ≪ surface (df=9). HONEST (adversarial review): (a) month split = calendar bin under NH naming, NOT hemisphere-aware season (CloudSEN12 spans both hemispheres); (b) `s2_sen2cor_version` = Sen2Cor product-version field, NOT the ESA Processing Baseline; (c) MIXING (measured): 158/184 components span both month-halves, 183/184 span >1 version → these arms are a LOOSE stratification that RECONFIRMS R17 random-subset robustness, NOT a clean cross-season/version transfer; surface (R17) is the cleanly-separable axis | `phase8R18_quantile_season_baseline.py` (10 model × 10 split seed) | `paper/results_phase8R18_quantile_season_baseline_{summary.json,percell.csv,log}` |
| Weighted CRC on re-normalized rep (R19, full 10×10, factorial) | src\_uniform 28.1@94 (reproduces flagship), src\_weighted 1.5@7 **feasible only 12%** (infeasible→deployment abstain-fallback, not a chosen operating point), ren\_uniform 9.4@71, ren\_weighted 8.9@69; **re-norm alone −18.7 [−19.7,−17.8]** restores the operating point, **weight\|renorm −0.5 [−1.0,+0.0] incl 0** (weighting adds no measurable value on the re-normalized rep); held-out AUROC 0.98→0.57, calib-ESS 10→75% (eval-ESS 39→67 — the earlier "41" mislabel was eval-ESS). Rewritten after adversarial review (mid-CDF ECDF, feasibility flag, calib-ESS via wc, factorial contrasts) | `phase8R19_weighted_crc_on_renorm.py` (10 model × 10 split seed) | `paper/results_phase8R19_weighted_crc_on_renorm_{summary.json,percell.csv,log}` |
