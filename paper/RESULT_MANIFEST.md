...
| P0-2 quantile-tie convention (adversarial review) | mid-CDF (tie-aware) vs released first-rank `np.interp` changes the transport joint by only **−0.03 pp** (max 0.06 pp over 5 splits, \|mean\| ≪ R10 SE 0.3) → **immaterial**; released R10/R17/R18 numbers stand, R19 already uses mid-CDF | probe: `scratchpad/probe_tie.py` | (verified, not a paper number) |
