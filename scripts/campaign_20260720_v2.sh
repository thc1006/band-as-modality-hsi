...
least_gpu() {  # index of the least-utilised GPU, sampled now
  nvidia-smi --query-gpu=index,utilization.gpu --format=csv,noheader,nounits \
    | sort -t, -k2 -n | head -1 | cut -d, -f1
}
clean_gpu() {  # a GPU with NO foreign compute process; empty if none (caller must decide)
  local mine ours
  ours=$(nvidia-smi --query-compute-apps=gpu_uuid --format=csv,noheader | sort -u)
...
run C phase8 "$(least_gpu)" 2 experiments/phase8_cloudsen12.py --seeds 0 1 2 3 4 --jobs 2
run C phase8D "$(least_gpu)" 2 experiments/phase8D_difficulty.py
run C phase8R "$(least_gpu)" 2 experiments/phase8R_reliability.py --jobs 2
run C phase8E "$(least_gpu)" 2 experiments/phase8E_dofa.py
run C phase8Fm "$(least_gpu)" 2 experiments/phase8F_multi.py
run C phase4R "$(least_gpu)" 2 experiments/phase4R_reliability.py

# LANE D -- efficiency timing, EXCLUSIVE (nothing else may run)
run D phase7 "$(clean_gpu)" 8 experiments/phase7_efficiency.py
...
# Propagate failure: any [FAIL] or a non-zero doctor makes the whole campaign non-zero, so a
# "finished" status can never sit over missing or stale outputs. See campaign_20260720.sh.
